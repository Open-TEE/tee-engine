...
static bool serialize_image(cmp_ctx_t *cmp,
			    TEE_TUIImage *image)
{
	/* There is no array element for image data if image is not present */
	if (!cmp_write_array(cmp, image->source == TEE_TUI_REF_SOURCE ? 4 : 3) ||
	    !cmp_write_uint(cmp, image->source))
		return false;

	/* TODO: Image source can be Object */
	if (image->source == TEE_TUI_REF_SOURCE)
		if (!cmp_write_bin(cmp, image->ref.image, image->ref.imageLength))
			return false;
			     ...
			     TEE_TUIButton *button)
{
	...
	if (!cmp_write_array(cmp, defaults ? 1 : 3) ||
	...
	/* text can be null */
	text = button->text == NULL ? "\0" : button->text;

	if (!cmp_write_str(cmp, text, strlen(text)) ||
	    !serialize_image(cmp, &button->image))
		return false;
