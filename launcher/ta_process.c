...
{
	/* empty test */
	//syslog(LOG_ERR, "add_msg_done_queue: %d\n", sig_vector);
}
 ...
 */
static void receive_from_manager(uint32_t events, int man_sockfd)
{
	struct ta_task *new_ta_task = NULL;
	int ret = -1;

	if (events & (EPOLLHUP | EPOLLERR)) {
		syslog(LOG_ERR, "receive_from_manager: Manger sock problem\n");
		exit(1);
	}
 ...
 * \param man_sockfd The socket connection to the manager
 */
static void reply_to_manager(uint32_t events, int man_sockfd)
{
	...
	if (events & (EPOLLHUP | EPOLLERR)) {
		syslog(LOG_ERR, "reply_to_manager: Event fd problem\n");
		exit(1);
	}
...
{
	/* Function is for developing purpose. It is implemented for assuring that there is no
	 * "obsolite" resources and all resources is handled properly */
...
static void ta_check_signal(struct epoll_event *event)
{
	if (event && event->events & (EPOLLHUP | EPOLLERR)) {
		syslog(LOG_ERR, "ta_check_signal: Self pipe fd error\n");
		epoll_unreg(event->data.fd);
		return;
	}

	if (sig_vector & TEE_SIG_TERM) {
		/* SIGTERM is signaling that manager or launcher is exited uncpectedly and
		 * therefore TA need to be closed properly */

		/* TODO: Call end session and destroy (First cancel current task and then call) */

		exit(EXIT_SUCCESS); /* For now */
	}
...
int ta_process_loop(int man_sockfd, struct com_msg_open_session *open_msg)
{
	...
	char proc_name[16]; /* For now */
	sigset_t sig_empty_set;

	if (sigemptyset(&sig_empty_set)) {
		syslog(LOG_ERR, "ta_process_loop: Sigempty set failed\n");
		exit(EXIT_FAILURE);
	}

	strncpy(proc_name, "tee_test_ta", 16);
	prctl(PR_SET_NAME, (unsigned long)proc_name);
	strncpy(argv0, proc_name, argv0_len); /* TODO: Why this is not working? */
	...
	/* Execute Create entry and open session functions */

	/* Known error: CA can not determ if TA is launched or not, because framework is calling
	 * create entry point and open session function those function return values is mapped into
	 * one return value. */
	execute_create_entry_point(man_sockfd, open_msg);

	/* NOTE: Not yet report to manager about create entry point function success value.
	 * Lets initialize TA process and report after when TA accepting messages */

	if (add_task_todo_queue_and_notify(open_ta_task)) /* Open session */
		exit(EXIT_FAILURE);
	...
	for (;;) {
		...
		if (event_count == -1) {
			if (errno == EINTR) {
				ta_check_signal(NULL);
				continue;
			}

			/* Log error and hope the error clears itself */
			syslog(LOG_ERR, "ta_process_loop: Failed return from epoll_wait\n");
			continue;
		}
		...
		for (i = 0; i < event_count; i++) {
			...
			if (cur_events[i].data.fd == exit_event_fd) {
				proc_cleanup(man_sockfd); /* Not needed but for developing purpose */
				exit(EXIT_SUCCESS);

			} else if (cur_events[i].data.fd == man_sockfd) {
				receive_from_manager(cur_events[i].events, man_sockfd);

			} else if (cur_events[i].data.fd == event_fd) {
				reply_to_manager(cur_events[i].events, man_sockfd);

			} else if (cur_events[i].data.fd == self_pipe_fd) {
				ta_check_signal(&cur_events[i]);
