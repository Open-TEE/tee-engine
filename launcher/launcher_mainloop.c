...
int lib_main_loop(struct core_control *ctl_params)
{
	...
	for (;;) {
		...
		for (i = 0; i < event_count; i++) {
			...
			ret = com_recv_msg(ctl_params->comm_sock_fd, (void **)&recv_open_msg, NULL, shm_fds, &shm_fd_count);
			...
			if (shm_fd_count > 0 && shm_fd_count<=4) {
				...
				memcpy(recv_open_msg->msg_hdr.shareable_fd, shm_fds, sizeof(int)*shm_fd_count);
			}
			...
			if (ret == sizeof(struct com_msg_ta_created)) {
				...
				if (send_fd(ctl_params->comm_sock_fd, &sockfd[0], 1, NULL, 0) == -1) {
