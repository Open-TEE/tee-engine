...
static const uint64_t COM_MSG_START = 0x00CCCCCCCCCCCCC0;
...
static int read_all(int fd, void *buf, int buf_len, void (*eintr_handler)())
{
	...
	for (i = 0; (i < TRY_READ_FD_COUNT && buf_len <= total_read_bytes); ++i) {
...
int com_recv_msg(int sockfd, void **msg, int *msg_len, void (*eintr_handler)())
{
	...
	*msg = NULL;
	...
	if (ret != -1 || ret != sizeof(struct com_trasnport_info)) {
	...
	if (com_recv_trans_info.start != COM_MSG_START) {
		/* TODO: Rewind socket to next start point, but now
		 * just return and hope that hte next read began with valid sequence */
...
int com_send_msg(int sockfd, void *msg, int msg_len)
{
	...
	if (bytes_write_trans_send == -1) {
		syslog(LOG_ERR, "com_write_msg: send error\n");
		return -1;
	}
