...
static void send_msg(proc_t send_to, void *msg, int msg_len)
{
	...
	if (com_send_msg(send_to->sockfd, msg, msg_len, msg_header->shareable_fd, msg_header->shareable_fd_count) != msg_len)
...
void read_fd_and_add_todo_queue(struct epoll_event *event)
{
	...
	/* Add message */
	ret = com_recv_msg(new_man_msg->proc->sockfd, &new_man_msg->msg, &new_man_msg->msg_len, NULL, NULL);
	if (ret == -1) {
