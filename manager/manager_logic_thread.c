...
static void launcher_status_change(int status)
{
	proc_t ta = NULL;

	if (WIFCONTINUED(status))
		syslog(LOG_ERR, "launcher_status_change: Recv continue sig\n"); /* No action needed */

	/* Signal caused stop. Note. No action, just logging */
	if (WIFSTOPPED(status))
		syslog(LOG_ERR, "launcher_status_change: recvstop signal\n");

	/* Terminated */
	if (WIFSIGNALED(status) || WIFEXITED(status)) {
		syslog(LOG_ERR, "launcher_status_change: Launcher terminated\n");

		/* It does not matter in which state launcher went down. Release file locks and
		 * send termination command to TAs and terminate manager */
		...
		while (1) {
			...
			release_ta_file_locks(ta);
			kill(ta->content.process.pid, SIGTERM);
		}
...
static void ta_status_change(proc_t ta, int status)
{
	proc_t link = NULL;

	if (WIFCONTINUED(status))
		syslog(LOG_ERR, "ta_status_change: Recv continue sig\n"); /* No action needed */

	/* Signal caused stop. Note. No action, just logging */
	if (WIFSTOPPED(status))
		syslog(LOG_ERR, "ta_status_change: recvstop signal\n");

	/* Signal caused termination */
	if (WIFSIGNALED(status)) {

		if (WTERMSIG(status) == SIGTERM) {
			syslog(LOG_ERR, "ta_status_change: ta recv sigterm\n");
			/* SIGTERM == TA is closing it self. No action needed */
		} else {
			syslog(LOG_ERR, "ta_status_change: ta recv some term sig\n");
			goto unknow_exit;
		}
	}
	...
	if (WIFEXITED(status)) {
		...
		if (WEXITSTATUS(status) == EXIT_SUCCESS) {
			syslog(LOG_ERR, "ta_status_change: ta exit_success\n");
			/* EXIT_SUCCESS == TA is closing it self. No action needed */
		} else {
			syslog(LOG_ERR, "ta_status_change: ta exit != exit_success\n");
			goto unknow_exit;
		}
	}
...
unknow_exit:
	...
	release_ta_file_locks(ta);
	ta->content.process.status = Panicked;
	epoll_unreg(ta->content.process.sockfd);

	/* Do not close socket. It is done in free_proc function */

	/* Mark all session as panics */
	h_table_init_stepper(ta->content.process.links);
	while (1) {
		link = h_table_step(ta->content.process.links);
		if (!link)
			break;

		link->content.sesLink.status = session_panicked;
	}
}

static void seek_and_destroy_dead_childs()
{
	proc_t changed_ta = NULL;
	pid_t changed_proc;
	int status;

	/* wait for children, to reap the zombies */
	while (1) {
		changed_proc = waitpid(-1, &status, WNOHANG);
		if (!changed_proc)
			break;

		/* If launcher is dead, gracefully kill of TAs with SIGTERM */
		if (changed_proc == launcher_pid)
			launcher_status_change(status);

		/* Get terminated TA */
		changed_ta = h_table_get(trustedApps, &changed_proc, sizeof(pid_t));
		ta_status_change(changed_ta, status);
	}
}

void manager_check_signals(struct epoll_event *event)
{
	if (event && event->events & (EPOLLHUP | EPOLLERR)) {
		syslog(LOG_ERR, "manager_check_signals: Self pipe fd error\n");
		epoll_unreg(event->data.fd);
		return;
	}

	if (sig_vector & TEE_SIG_CHILD) {
		/* Know error: If Signal handler is invoked here, we could miss one sig child signal */
		sig_vector ^= TEE_SIG_CHILD;
		seek_and_destroy_dead_childs();
	}
...
static int comm_launcher_to_launch_ta(void *msg, int msg_len, int *new_ta_fd, pid_t *new_ta_pid)
{
	...
	if (recv_created_msg->pid == -1) {
		syslog(LOG_ERR, "comm_launcher_to_launch_ta: Problem in TA launching\n");
		free(recv_created_msg);
		return 1;
	}
...
static void close_session(struct manager_msg *man_msg)
{
	...
	if (close_msg->should_ta_destroy)
		man_msg->proc->content.sesLink.owner->content.process.status = Disconnected;
