...
static uint32_t close_object(uint32_t storage_blob_id, void *objectID, size_t objectIDLen)
{
	struct list_head *pos;
	struct storage_element *current_element = NULL;

	if (!IS_VALID_STORAGE_BLOB(storage_blob_id)) {
		OT_LOG(LOG_ERR, "Not a valid storage object");
		return 1;
	}
...
uint32_t ext_delete_storage_blob(uint32_t storage_blob_id, void *objectID, size_t objectIDLen)
{
	...
	if (object_file && close_object(storage_blob_id, objectID, objectIDLen)) {
		return 1;
			  ...
			  size_t new_objectIDLen)
{
	...
	if (access(new_name_with_dir_path, F_OK) == 0) {
		OT_LOG(LOG_ERR, "Cannot change object ID, because new ID is in use [%d]", errno);
		ret_val = false;
		goto exit;
	}
