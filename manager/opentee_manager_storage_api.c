...
static TEE_Result alloc_storage_path(void *objectID, size_t objectIDLen, char **name_with_dir_path, char **return_dir_path)
{
	size_t i;
...
TEE_Result MGR_TEE_WriteObjectData(TEE_ObjectHandle object, void *buffer, size_t size)
{
	...
	if (!(object->objectInfo.handleFlags & (TEE_DATA_FLAG_ACCESS_WRITE | TEE_DATA_FLAG_ACCESS_WRITE_META))) {
