					  ...
					  size_t initialDataLen)
{
	...
	// write initi data
	if (initialData) {
		if (initialDataLen != ext_write_stream(ss_id, object_meta_info.data_begin, initialData, initialDataLen)) {
				   ...
				   size_t pos)
{
	...
	//Total size includes meta and attributes
 	ss_cur_total_size = object_meta_info.data_begin + object_meta_info.data_size;
	ss_pos_total_size = object_meta_info.data_begin + pos + size;
	...
	if (ss_pos_total_size > ss_cur_total_size) {
		...
		rv = MGR_TEE_TruncateObjectData(objectID, objectIDLen, ss_pos_total_size);
		if (rv != TEE_SUCCESS) {
	...
	write_bytes = ext_write_stream(object->per_object.storage_blob_id,
				       object->per_object.data_begin + pos,
				       buffer, size);
	if (write_bytes != size) {
...
TEE_Result MGR_TEE_TruncateObjectData(void *objectID, uint32_t objectIDLen, size_t size)
{
	TEE_Result rv;
	uint32_t storage_blob_id;
	struct ss_object_meta_info object_meta_info;
