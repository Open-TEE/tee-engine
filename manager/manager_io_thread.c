...
static void add_msg_to_queue(proc_t ptr)
{
	...
	/* Message received succesfully or not. Add message "sender" details */
	new_man_msg->proc = ptr;

	if (ptr->p_type == sessionLink)
		ret = com_recv_msg(ptr->content.sesLink.sockfd, &new_man_msg, &new_man_msg->msg_len, &handle_sig);
	else
		ret = com_recv_msg(ptr->content.process.sockfd, &new_man_msg, &new_man_msg->msg_len, &handle_sig);
