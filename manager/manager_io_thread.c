...
void handle_done_queue(uint32_t events)
{
	struct manager_msg *handled_msg;
	uint64_t event;

	if (events & (EPOLLHUP | EPOLLERR)) {
		syslog(LOG_ERR, "handle_done_queue: Event fd error");
		exit(1);
		/* TODO: Re open or exit. If this is not working, no message is delivered! */
	}
...
static void add_msg_to_queue(proc_t ptr)
{
	...
	new_man_msg->proc = ptr;
	  ...
	  if (ptr->p_type == sessionLink)
		ret = com_recv_msg(ptr->content.sesLink.sockfd, &new_man_msg->msg, &new_man_msg->msg_len, NULL);
	else
		ret = com_recv_msg(ptr->content.process.sockfd, &new_man_msg->msg, &new_man_msg->msg_len, NULL);
...
void handle_public_fd(uint32_t events, int pub_fd)
{
	...
	if (events & (EPOLLHUP | EPOLLERR)) {
		syslog(LOG_ERR, "handle_public_fd: Public socket error");
		epoll_unreg(pub_fd);
		close(pub_fd);
		/* TODO: Re open pub fd */
		return;
...
void pm_handle_connection(uint32_t events, void *proc_ptr)
{
	...
	if (events & (EPOLLHUP | EPOLLERR)) {
		 ...
		 * epoll listener (and explicitedly close this end of the socket. Close in handler) */
		if (ptr->p_type == sessionLink) {
			epoll_unreg(ptr->content.sesLink.sockfd);
			//close(ptr->sesLink.sockfd);
		} else {
			epoll_unreg(ptr->content.process.sockfd);
			//close(ptr->process.sockfd);
		}
