...
static void remove_node_from_list(struct ta_metadata *remove_node)
{
	pthread_mutex_lock(&list_mutex);
...
int remove_metadata(char *elf_file)
{
	...
	LIST_FOR_EACH(pos, head) {
		...
		if (strcmp(remove_node->elf_file_name, elf_file) == 0) {
			remove_node_from_list(remove_node);
			return 0;
		}
	}
...
{
	/* IN_MOVE_SELF event for a directory does not trigger event for
	 * files in it. Therefore need to clear metadata for them individually
	 */
	if (head) {
		while (!list_is_empty(head))
			remove_node_from_list(LIST_ENTRY(head->next, struct ta_metadata, list));
	}
