...
static void invoke_cmd_add_fds(struct com_msg_operation *operation, struct com_msg_hdr *msg_hdr, struct proc_shm_mem* proc_shm_mem)
{
	...
	for (i = 0; i < 4; ++i) {
		...
		if (TEE_PARAM_TYPE_GET(param_types, i) == TEEC_NONE ||
			...
			LIST_FOR_EACH(pos, &proc_shm_mem->list) {
				...
				if (strcmp(current_shm->name, operation->params[i].param.memref.shm_area) == 0)
				{
...
static void open_session_msg(struct manager_msg *man_msg)
{
	...
	if (open_msg->msg_hdr.msg_type == COM_TYPE_QUERY) {
		invoke_cmd_add_fds(&open_msg->operation,&open_msg->msg_hdr, &man_msg->proc->shm_mem);
...
static void invoke_cmd(struct manager_msg *man_msg)
{
	...
	if (invoke_msg->msg_hdr.msg_type == COM_TYPE_QUERY) {
		...
		invoke_cmd_add_fds(&invoke_msg->operation,&invoke_msg->msg_hdr, &man_msg->proc->shm_mem);
