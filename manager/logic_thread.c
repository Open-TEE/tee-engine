...
static void free_proc(proc_t del_proc) { del_proc = del_proc; }
...
static void free_sess(proc_t del_sess) { del_sess = del_sess; }
...
static void remove_session_between(proc_t owner, proc_t to, uint64_t sess_id)
{
	...
	free_sess(
	    h_table_get(to->content.process.links, (unsigned char *)(&sess_id), sizeof(uint64_t)));
}
