					    ...
					    struct com_mgr_invoke_cmd_payload *out)
{
	...
	if (createMessage->initialDataLen > 0) {
		initialData = (uint8_t *)in->data + sizeof(struct com_mrg_create_persistent) + createMessage->attributeSize;
	}
					 ...
					 uint8_t write_type)
{
	...
	actualWriteSize = inTransferMsg->dataSize;
