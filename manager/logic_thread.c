...
static void free_proc(proc_t del_proc)
{
	...
	if (del_proc->p_type == proc_t_CA && pthread_mutex_lock(&CA_table_mutex)) {
		OT_LOG(LOG_ERR, "Failed to lock the mutex");
		return;
	}
	...
	if (del_proc->p_type == proc_t_CA)
		h_table_remove(clientApps, (unsigned char *)&del_proc->sockfd,
					   sizeof(del_proc->sockfd));

	/* Trusted process spesific operations */
	if (del_proc->p_type == proc_t_TA) {
		release_ta_file_locks(del_proc);
		h_table_remove(trustedApps, (unsigned char *)&del_proc->content.process.pid, sizeof(pid_t));
	}
	...
	if (del_proc->p_type == proc_t_CA && pthread_mutex_unlock(&CA_table_mutex))
		OT_LOG(LOG_ERR, "Failed to unlock the mutex");
OT_LOG_STR("PROC DELL")
	free(del_proc);
	del_proc = NULL;
}
