				       ...
				       struct com_mgr_invoke_cmd_payload *out)
{
	...
	MGR_TEE_CloseObject(closeObject->ss_id);
					    ...
					    struct com_mgr_invoke_cmd_payload *out)
{
	...
	size_t initialDataLen = 0;
	uint32_t ss_id = 0;

	if (in->size > offsetof(struct com_mrg_create_persistent, initialDataOffset)) {

		attrs = calloc(createMessage->attrs_count, sizeof(TEE_Attribute));
		if (attrs == NULL) {
			ret = TEE_ERROR_OUT_OF_MEMORY;
			goto out;
		}

		unpack_object_attrs(attrs,
				    createMessage->attrs_count,
				    &createMessage->attributeHandleOffset);
	}

	ret = MGR_TEE_CreatePersistentObject(createMessage->storageID, createMessage->objectID,
					     createMessage->objectIDLen, createMessage->flags,
					     attrs, createMessage->attrs_count,
					     createMessage->data_object, &ss_id,
					     &createMessage->info,
	...
	if (ret == TEE_SUCCESS) {
		...
		((struct com_mrg_create_persistent *)out->data)->ss_id = ss_id;
		memcpy(((struct com_mrg_create_persistent *)out->data)->info, &createMessage->info, sizeof(TEE_ObjectHandle));
	}
