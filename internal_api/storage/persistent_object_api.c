...
static TEE_Result write_object_data(uinit32_t ss_id,
				    void *buffer,
				    size_t size,
				      ...
				      TEE_ObjectHandle *object)
{
	...
	messagaSize = offsetof(struct com_mrg_create_persistent, initialDataOffset);
	messagaSize += initialDataLen;
	if (attributes) {
	...
	/* Only return value is storage blob ID and objectInfo */
	tempHandle->per_object.storage_blob_id =
			((struct com_mrg_create_persistent *)returnPayload.data)->ss_id;
	memcpy(&tempHandle->objectInfo,
	       ((struct com_mrg_create_persistent *)returnPayload.data)->info,
	       sizeof(TEE_ObjectInfo));
	
	/*	
	if (initialDataLen > 0) {

		ret = write_object_data(tempHandle->per_object.storage_blob_id,
					initialData,
					initialDataLen,
					COM_MGR_CMD_ID_WRITE_CREATE_INIT_DATA);

		if (ret != TEE_SUCCESS) {
			goto err_2; //Error msg logged
		}
	}
	*/
