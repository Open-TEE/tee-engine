			    ...
			    uint32_t maxObjectSize)
{
	...
	if (malloc_attr(&key->gp_attrs.attrs[index++], TEE_ATTR_RSA_MODULUS, rsa_comp_size, NULL) ||
	    malloc_attr(&key->gp_attrs.attrs[index++], TEE_ATTR_RSA_PUBLIC_EXPONENT, rsa_comp_size, NULL)) {
		goto err;
	}
	...
	if (malloc_attr(&key->gp_attrs.attrs[index++], TEE_ATTR_RSA_PRIVATE_EXPONENT, rsa_comp_size, NULL) ||
	    malloc_attr(&key->gp_attrs.attrs[index++], TEE_ATTR_RSA_PRIME1, rsa_comp_size, NULL) ||
	    malloc_attr(&key->gp_attrs.attrs[index++], TEE_ATTR_RSA_PRIME2, rsa_comp_size, NULL) ||
	    malloc_attr(&key->gp_attrs.attrs[index++], TEE_ATTR_RSA_EXPONENT1, rsa_comp_size, NULL) ||
	    malloc_attr(&key->gp_attrs.attrs[index++], TEE_ATTR_RSA_EXPONENT2, rsa_comp_size, NULL) ||
	    malloc_attr(&key->gp_attrs.attrs[index++], TEE_ATTR_RSA_COEFFICIENT, rsa_comp_size, NULL)) {
		goto err;
	}
				  ...
				  uint32_t paramCount)
{
	...
	rv_mbedtls = mbedtls_rsa_export(&key->key.rsa.ctx, &N, &P, &Q, &D, &E);
	if (rv_mbedtls != 0) {
		print_mbedtls_to_syslog(rv_mbedtls);
	...
	rv_mbedtls = mbedtls_rsa_export_crt(&key->key.rsa.ctx, &DP, &DQ, &QP);
	if (rv_mbedtls != 0) {
		print_mbedtls_to_syslog(rv_mbedtls);
