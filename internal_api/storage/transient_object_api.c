				   ...
				   struct gp_key *key)
{
	...
	if (key->gp_key_type == TEE_TYPE_RSA_KEYPAIR) {
		...
		if (private_exp->content.ref.length != mbedtls_RSA_PRIVATE_EXP(modulo->content.ref.length)) {
			OT_LOG_ERR("Panicking due private exponent size mismatch: reserved size [%u]; provided size [%u]\n", private_exp->content.ref.length, mbedtls_RSA_PRIVATE_EXP(modulo->content.ref.length));
			return TEE_ERROR_BAD_PARAMETERS;
		}
				...
				uint32_t maxObjectSize)
{
	*key = (struct gp_key *)calloc(1, sizeof(struct gp_key));
	...
	case TEE_TYPE_RSA_KEYPAIR:
		...
		//mbedtls_rsa_init(&(*key)->key.rsa.ctx, 0, 0);
				  ...
				  uint32_t paramCount)
{
	...
	if (usr_public_exp) {
		...
		TEE_MemMove(&pub_exp, usr_public_exp->content.ref.buffer, mbedtls_RSA_PUBLIC_EXP);
	}
	...
	//TODO: Copy mbedtls compinents to GP key
				       ...
				       TEE_ObjectHandle *object)
{
	...
	*object = 0;
	...
	if (valid_object_type_and_max_size(objectType, maxObjectSize)) {
		OT_LOG_ERR("TEE_AllocateTransientObject not supported objectType [%u] OR/AND objectMaxSize [%u]", objectType, maxObjectSize);
...
void TEE_FreeTransientObject(TEE_ObjectHandle object)
{
	...
	if (object->objectInfo.handleFlags & TEE_HANDLE_FLAG_PERSISTENT) {
		OT_LOG_ERR("Panicking due trying to free persistant object\n");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}
