...
static int32_t BigIntToBN(BIGNUM *dest, TEE_BigInt *src)
{
	...
	BIGNUM *ret = BN_bin2bn(GetData(src), GetHeader(src)->length, dest);
	...
	BN_set_negative(ret, GetHeader(src)->neg);
...
static int32_t BNToBigInt(TEE_BigInt *dest, const BIGNUM *src)
{
	...
	struct TEE_BigInt_InternalHeader *header = GetHeader(dest);

	size_t representation_length = BN_num_bytes(src) / sizeof(uint32_t);

	/* Check if representation fits in given TEE_BigInt */
	if (header->length < representation_length) {
		printf("Does not fit %i < %zu\n", header->length, representation_length);
		/* TODO: Some internal error constants */
		return -1;
	}
	...
	unsigned char *data = GetData(dest);
		   ...
		   TEE_BigInt *op1, TEE_BigInt *op2)
{
	...
	if (BN_is_zero(bn_op2)) {
		printf("Panic Reason op2 == 0, Division by zero");
		/* TODO: Check correct panic code */
				  ...
				  TEE_BigInt *op2)
{
	...
	/* Allocate BIGNUMs for parameters */
	BIGNUM *bn_op1 = AllocAndConvert(op1);
	BIGNUM *bn_op2 = AllocAndConvert(op2);
	...
	BIGNUM *bn_t = BN_new();
	BIGNUM *bn_r = BN_new();
	BIGNUM *bn_old_s = BN_new();
	BIGNUM *bn_old_t = BN_new();
	BIGNUM *bn_old_r = BN_new();
	BIGNUM *bn_quotient = BN_new();
	BIGNUM *bn_temp = BN_new();

	if (bn_op1 == NULL || bn_op2 == NULL ||
	    bn_s == NULL || bn_t == NULL || bn_r == NULL ||
	    bn_old_s == NULL || bn_old_t == NULL || bn_old_r == NULL ||
	    bn_quotient == NULL || bn_temp == NULL)
		goto err2;
	...
	BN_set_word(bn_s, 0);
	BN_set_word(bn_t, 1);
	BN_set_word(bn_old_s, 1);
	BN_set_word(bn_old_t, 0);

	while (!BN_is_zero(bn_r)) {
		/* Quotient <= old_r / r */
		if (!BN_div(bn_quotient, NULL, bn_old_r, bn_r, context))
			goto err3;

		/* (old_r, r) <= (r, old_r - quotient * r) */
		BIGNUM *tmp = bn_r;

		if (!BN_mul(bn_temp, bn_quotient, bn_r, context))
			goto err3;

		if (!BN_sub(bn_old_r, bn_old_r, bn_temp))
			goto err3;
		...
		if (!BN_mul(bn_temp, bn_quotient, bn_s, context))
			goto err3;

		if (!BN_sub(bn_old_s, bn_old_s, bn_temp))
			goto err3;
		...
		if (!BN_mul(bn_temp, bn_quotient, bn_t, context))
			goto err3;

		if (!BN_sub(bn_old_t, bn_old_t, bn_temp))
			goto err3;
	...
	/* Convert result BIGNUM -> TEE_BigInt */
	if (BNToBigInt(gcd, bn_old_r))
		goto err3;

	if (u != NULL)
		if (BNToBigInt(u, bn_old_s))
			goto err3;

	if (v != NULL)
		if (BNToBigInt(v, bn_old_t))
			goto err3;
...
err2:
	...
	BN_clear_free(bn_quotient);
	BN_clear_free(bn_old_r);
	BN_clear_free(bn_old_t);
	BN_clear_free(bn_old_s);
	BN_clear_free(bn_r);
	BN_clear_free(bn_t);
	BN_clear_free(bn_s);
	BN_clear_free(bn_op2);
