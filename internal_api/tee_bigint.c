	...
	if (representation_length == 0)
		...
		*dest = 0;
		...
		BN_bn2bin(bn_src,
                ...
                *dest = ntohl(*dest) * (BN_is_negative(bn_src) ? -1 : 1);
        }
	else
        {
		printf("Too big representation, would require %zu bytes", representation_length);
			    ...
			    TEE_BigInt *n, TEE_BigIntFMMContext *context)
{
	...
	const size_t src_len =
            (GetHeader(src)->allocated_u32s * sizeof(uint32_t)) +
