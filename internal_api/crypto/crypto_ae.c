			      ...
			      void *tag, size_t *tagLen)
{
	...
	if (operation == NULL) {
		...
		OT_LOG_ERR("TEE_AE_FInal panics due tagLen not right (expected[%u], provided[%u])",
	...
	if (srcData != NULL) {
		rv_gp = TEE_AEUpdate(operation, srcData, srcData, destData, destLen);
	}
	...
	if (destLen != NULL) {
		destDataLeftStart = (uint8_t *)destData + *destLen;
		destDataLeftLen -= *destLen;
	}
		     ...
		     void *AADdata, size_t AADdataLen)
{
	...
	rv_mbedtls = mbedtls_gcm_update_ad(operation->ctx, AADdata, AADdataLen);
	if (rv_mbedtls) {
		print_mbedtls_to_syslog(rv_mbedtls);
		OT_LOG(LOG_ERR,"Error: Internal AE error");
		return TEE_ERROR_BAD_PARAMETERS;
			...
			void *destData, size_t *destLen)
{
	...
	if (operation == NULL) {
		...
		OT_LOG_ERR("TEE_AEUpdate panics due destLen too short (srcLen[%u]; "
			   "destLen[%u])", srcLen, *destLen);
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}
