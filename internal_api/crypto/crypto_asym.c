				     ...
				     void *signature, size_t *signatureLen)
{
	...
	uint8_t ecc_d_p256[] = "\x51\x9b\x42\x3d\x71\x5f\x8b\x58\x1f\x4f\xa8\xee\x59\xf4\x77\x1a\x5b\x44\xc8\x13\x0b\x4e\x3e\xac\xca\x54\xa5\x6d\xda\x72\xb4\x64";
	...
	if (rv_mbedtls != 0) {
		...
		OT_LOG_ERR("ERROR: failure internal crypto error key pairing(ECC calc)");
		return TEE_ERROR_GENERIC;
	}
	OT_LOG_HEX_BUF("sig0", signature, sig_len);
	

	rv_mbedtls = mbedtls_ecdsa_sign_det_ext(&grp, &r, &s, &operation->ctx.ecc.ctx->private_d,
						//&operation->ctx.ecdsa.ctx->private_d,
						digest, digestLen,
						MBEDTLS_MD_SHA256,
						mbedtls_ctr_drbg_random,
						&ot_mbedtls_ctr_drbg);
	...
	sigLen = mbedtls_mpi_size(&s);
	mbedtls_mpi_write_binary(&s, sig, sigLen);
	OT_LOG_HEX_BUF("sig", sig, sigLen);

	rv_mbedtls = mbedtls_ecdsa_sign(&grp, &r, &s, &operation->ctx.ecc.ctx->private_d,
					//&operation->ctx.ecdsa.ctx->private_d,
					digest, digestLen,
					mbedtls_ctr_drbg_random,
					&ot_mbedtls_ctr_drbg);
	...
	sigLen = mbedtls_mpi_size(&s);
	mbedtls_mpi_write_binary(&s, sig, sigLen);
	OT_LOG_HEX_BUF("sig2", sig, sigLen);


	rv_mbedtls = mbedtls_ecdsa_verify(&grp, digest, digestLen, &operation->ctx.ecc.ctx->private_Q, &r, &s);
	if (rv_mbedtls != 0) {
		print_mbedtls_to_syslog(rv_mbedtls);
		OT_LOG_ERR("ERROR: failure internal crypto error key pairing(ECC calc)");
		return TEE_ERROR_GENERIC;
				     ...
				     void *signature, size_t signatureLen)
{
	...
	if (rv_mbedtls != 0) {
		print_mbedtls_to_syslog(rv_mbedtls);
		OT_LOG_ERR("ERROR: failure internal crypto error key pairing(ECC importing)");
		return TEE_ERROR_GENERIC;
	}
