...
{
	if (digestLen > (operation->key->key_lenght - 11))
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);

	if (operation->key->key_lenght > *signatureLen)
	...
	*signatureLen = operation->key->key_lenght;
...
{
	if (digestLen > (operation->key->key_lenght - 11) ||
	    operation->key->key_lenght != signatureLen)
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
				      ...
				      uint32_t *destLen)
{
	...
	if (srcLen == 0 || srcLen > (operation->key->key_lenght - 11))
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);

	if (operation->key->key_lenght > *destLen)
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	...
	*destLen = operation->key->key_lenght;
	return TEE_SUCCESS;
}
