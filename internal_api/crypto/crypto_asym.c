...
//OT_LOG_HEX_BUF("signature", signature, *signatureLen);
	...
	rv_mbedtls = mbedtls_rsa_rsassa_pkcs1_v15_sign(&operation->key_data->key.rsa.ctx,
						       ...
						       &ot_mbedtls_ctr_drbg,
						       MBEDTLS_MD_SHA1, //map_gp_pkcs_hash(operation->operation_info.algorithm),
						       20, //get_alg_hash_lenght(operation->operation_info.algorithm),
						       digest,
						       signature);
...
{	
	...
	rv_mbedtls = mbedtls_rsa_set_padding((mbedtls_rsa_context *)&operation->key_data->key.rsa,
				      ...
				      void *destData, size_t *destLen)
{
	...
	OT_LOG_HEX_BUF("srcData", srcData, srcLen);
	OT_LOG_HEX_BUF("destData", destData, *destLen);
				 ...
				 void *destData, size_t *destLen)
{
	...
	OT_LOG_ERR("TEE_AsymmetricDecrypt called");
