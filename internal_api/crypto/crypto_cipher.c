			  ...
			  uint32_t mode)
{
	...
	new_cipher_ctx = (struct cipher_ctx *)calloc(1, sizeof(struct cipher_ctx));
	...
	case TEE_ALG_AES_CBC_NOPAD:
		...
		new_cipher_ctx->ctx = calloc(1, MAX_CIPHER_IV_LENGTH);
		if (new_cipher_ctx->ctx == NULL)
			goto out_of_mem_2;

		new_cipher_ctx->IV = calloc(1, MAX_CIPHER_IV_LENGTH);
