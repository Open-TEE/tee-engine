			     ...
			     uint32_t srcLen, void *destData, uint32_t *destLen)
{
	...
	int mbedret;
	uint32_t updateDestLen = 0, finalDestLen = 0;
	void *leftDestData;
	...
	OT_LOG_ERR("finalDestLen %u", finalDestLen);
	...
	OT_LOG_ERR("finalDestLen %u", finalDestLen);
	OT_LOG_ERR("*destLen %u", *destLen);
	OT_LOG_ERR("updateDestLen %u", updateDestLen);
	
	//	*destLen = updateDestLen + finalDestLen;

	OT_LOG_ERR("*destLen2 %u", *destLen);
	TEE_ResetOperation(operation);
