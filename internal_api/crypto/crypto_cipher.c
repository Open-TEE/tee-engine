		    ...
		    void *IV, size_t IVLen)
{
	...
	if (mbedtls_cipher_setkey(cipher_ctx, operation->key_data->key.secret.key,
		...
		OT_LOG(LOG_ERR, "TEE_CipherInit panics due unable set key");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}

	if (mbedtls_cipher_set_iv(cipher_ctx, (uint8_t *)IV, IVLen)) {
		OT_LOG(LOG_ERR, "TEE_CipherInit panics due unable set IV");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}
	...
	memset(operation->key_data->key.secret.genericBuf, 0, GENERIC_BUF_LEN);
	operation->key_data->key.secret.genericVar = 0;
			    ...
			    void *destData, size_t *destLen)
{
	...
	if (operation == NULL) {
		...
		OT_LOG_ERR("TEE_CipherUpdate panics due srcLen bigger than destLen");
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}

	rv_mbedtls = mbedtls_cipher_update((mbedtls_cipher_context_t *)operation->ctx,
	...
	OT_LOG_HEX_BUF("plain", srcData, srcLen);
	OT_LOG_HEX_BUF("cipher", destData, *destLen);
