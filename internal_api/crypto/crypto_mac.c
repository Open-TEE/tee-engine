...
static void do_hmac_set_key(TEE_OperationHandle operation)
{
	if (mbedtls_md_hmac_starts((mbedtls_md_context_t *)operation->ctx,
				   operation->key->key.secret.key,
				   operation->key->key_lenght)) {
...
TEE_Result mac_gp_init(TEE_OperationHandle operation, uint32_t algorithm)
{
...
void mac_gp_free(TEE_OperationHandle operation)
{
