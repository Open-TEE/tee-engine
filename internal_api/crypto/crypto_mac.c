...
static void do_hmac_set_key(TEE_OperationHandle operation)
{
	...
	rv_mbedtls = mbedtls_md_hmac_starts((mbedtls_md_context_t *)operation->ctx,
...
static void do_hmac_update(TEE_OperationHandle operation, uint8_t *chunk, uint32_t chunkSize)
{
	...
	rv_mbedtls = mbedtls_md_hmac_update((mbedtls_md_context_t *)operation->ctx,
					    chunk, chunkSize);
	if (rv_mbedtls) {
		print_mbedtls_to_syslog(rv_mbedtls);
		OT_LOG(LOG_ERR, "Error: internal crypto (MAC)");
