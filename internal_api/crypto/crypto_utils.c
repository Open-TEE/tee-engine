				   ...
				   TEE_OperationHandle operation)
{
	...
	if ((operation->operation_info.operationClass == TEE_OPERATION_CIPHER ||
		...
		OT_LOG_ERR("OperationKeyError: Cipher and Mac operation handle already TEE_HANDLE_FLAG_INITIALIZED (operationClass[%u])",
			   operation->operation_info.operationClass);
		return TEE_ERROR_BAD_PARAMETERS;
	}

	if (operation->operation_info.handleState & TEE_HANDLE_FLAG_KEY_SET) {
		OT_LOG_ERR("OperationKeyError: Operation has already key set");
		return TEE_ERROR_BAD_PARAMETERS;
	}
