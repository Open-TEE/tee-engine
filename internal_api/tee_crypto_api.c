...
static bool alg_requires_2_keys(uint32_t alg) { return (alg == TEE_ALG_AES_XTS) ? true : false; }
...
static TEE_Result malloc_and_cpy_rsa_key(TEE_OperationHandle operation, TEE_ObjectHandle key)
{
	...
	if (key->objectInfo.objectType == TEE_TYPE_RSA_KEYPAIR) {
		...
		if (!cpy_key_comp_to_bn(&RSA_key(operation)->d, TEE_ATTR_RSA_PRIVATE_EXPONENT,
					key)) {
		...
		if (get_attr_by_ID(key, TEE_ATTR_RSA_PRIME1) ||
			...
			if (!cpy_key_comp_to_bn(&RSA_key(operation)->p, TEE_ATTR_RSA_PRIME1, key) ||
			    ...
			    !cpy_key_comp_to_bn(&RSA_key(operation)->dmp1, TEE_ATTR_RSA_EXPONENT1,
						key) ||
			    !cpy_key_comp_to_bn(&RSA_key(operation)->dmq1, TEE_ATTR_RSA_EXPONENT2,
						key) ||
			    !cpy_key_comp_to_bn(&RSA_key(operation)->iqmp, TEE_ATTR_RSA_COEFFICIENT,
						key)) {
				      ...
				      uint32_t paramCount, const EVP_MD *hash)
{
	...
	if (-1 == beta_RSA_padding_check_PKCS1_OAEP_mgf1(
		      destData, *destLen, (unsigned char *)srcData + 1, srcLen - 1,
		      RSA_size(RSA_key(operation)), oaep_label, oaep_label_len, hash, NULL)) {
...
static TEE_Result rsa_op_generic_pre_checks_and_setup(
    TEE_OperationHandle operation, const EVP_MD **hash, void *srcData, uint32_t srcLen,
    void *destData, uint32_t *destLen, unsigned char **rsa_mod_len_buf, uint32_t *rsa_mod_len)
{
...
void TEE_CopyOperation(TEE_OperationHandle dstOperation, TEE_OperationHandle srcOperation)
{
	...
	} else if (srcOperation->operation_info.operationClass ==
...
void TEE_CipherInit(TEE_OperationHandle operation, void *IV, uint32_t IVLen)
{
	...
	if (operation->operation_info.mode == TEE_MODE_ENCRYPT) {
		...
		if (EVP_EncryptInit_ex(SYM_ctx(operation), NULL, NULL, SYM_key(operation), IV) ==
		    0) {
			syslog(LOG_ERR, "TEE_CipherInit: Enc operation init (openssl failure)\n");
			ret = TEE_ERROR_GENERIC;
			goto err;
		...
		if (EVP_DecryptInit_ex(SYM_ctx(operation), NULL, NULL, SYM_key(operation), IV) ==
		    0) {
			syslog(LOG_ERR, "TEE_CipherInit: Dec operation init (openssl failure)\n");
			ret = TEE_ERROR_GENERIC;
			goto err;
		   ...
		   TEE_ObjectHandle derivedKey)
{
	...
	bn_dh_pub_val = BN_bin2bn(dh_pub_val->content.ref.buffer, dh_pub_val->content.ref.length,
				  bn_dh_pub_val);
	if (!bn_dh_pub_val) {
	...
	shared_secret_size =
	    DH_compute_key(der_key_gen_sec->content.ref.buffer, bn_dh_pub_val, DH_key(operation));
