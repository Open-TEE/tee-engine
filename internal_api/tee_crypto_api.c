...
static uint32_t rsa_msg_max_len(TEE_OperationHandle operation, const EVP_MD *hash)
{
	...
	default:
		/* Should never end up here */
		syslog(LOG_ERR, "rsa_msg_max_len: Algortigm not supported\n");
		TEE_Panic(TEE_ERROR_GENERIC);
	}
...
//if (!(operation->operation_info.handleState & TEE_HANDLE_FLAG_KEY_SET)) printf("JOU\n");
...
/* GB TEE AE API is not supported */
