				    ...
				    uint32_t flags, TEE_ObjectHandle *object)
{
	...
	if (payload.data) {
		struct com_mrg_open_persistent *openParams = payload.data;
		...
		if (retVal == TEE_SUCCESS && returnPayload.size > 0) {
			...
			if (initialDataLen > 0 || object) {
				...
				if (initialDataLen > 0) {
					...
					retVal = TEE_WriteObjectData(tempHandle, initialData, initialDataLen);
...
void TEE_FreePersistentObjectEnumerator(TEE_ObjectEnumHandle objectEnumerator)
{
	...
	retVal = TEE_InvokeMGRCommand(TEE_TIMEOUT_INFINITE, COM_MGR_CMD_ID_OBJ_ENUM_FREE_PERSIST,
				      &payload, &returnPayload);
...
void TEE_ResetPersistentObjectEnumerator(TEE_ObjectEnumHandle objectEnumerator)
{
	...
	if (payload.data) {
		...
		retVal = TEE_InvokeMGRCommand(TEE_TIMEOUT_INFINITE, COM_MGR_CMD_ID_OBJ_ENUM_RESET_PERSIST,
						  &payload, &returnPayload);
					       ...
					       uint32_t storageID)
{
	...
	enumParams = payload.data;
	enumParams->ID = objectEnumerator->ID;
	memcpy(payload.data + sizeof(struct com_mrg_enum_command), &storageID, sizeof(storageID));

	retVal = TEE_InvokeMGRCommand(TEE_TIMEOUT_INFINITE, COM_MGR_CMD_ID_OBJ_ENUM_START, &payload,
				      &returnPayload);
				       ...
				       size_t *objectIDLen)
{
	...
	if (payload.data) {
		...
		retVal = TEE_InvokeMGRCommand(TEE_TIMEOUT_INFINITE, COM_MGR_CMD_ID_OBJ_ENUM_GET_NEXT,
					      &payload, &returnPayload);
...
TEE_Result TEE_WriteObjectData(TEE_ObjectHandle object, void *buffer, size_t size)
{
	...
	retVal = TEE_InvokeMGRCommand(TEE_TIMEOUT_INFINITE, COM_MGR_CMD_ID_WRITE_OBJ_DATA, &payload,
				      &returnPayload);
...
TEE_Result TEE_TruncateObjectData(TEE_ObjectHandle object, uint32_t size)
{
	...
	retVal = TEE_InvokeMGRCommand(TEE_TIMEOUT_INFINITE, COM_MGR_CMD_ID_TRUNCATE_OBJ_DATA,
				      &payload, &returnPayload);
