				 ...
				 uint32_t maxKeySize)
{
	...
	tmp_handle->operation_info.numberOfKeys = 0;
	...
	case TEE_ALG_AES_GCM:
		ret = init_gp_ae_cipher(tmp_handle);
		if (ret != TEE_SUCCESS)
			goto err;
...
{
	TEE_OperationInfoMultiple rv_info;
			       ...
			       TEE_ObjectHandle key)
{
	...
	if (operation == NULL) {
		...
		OT_LOG_ERR("TEE_SetOperationKey panicking due operation initilaized (contains TEE_HANDLE_FLAG_INITIALIZED)"); 
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}
