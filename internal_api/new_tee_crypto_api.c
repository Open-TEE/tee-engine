				 ...
				 uint32_t maxKeySize)
{
	...
	if (valid_mode_and_algorithm(algorithm, mode)) {
		OT_LOG_ERR("TEE_AllocateOperation algorithm [%u] and mode [%u] combination is not valid", algorithm, mode);
		return TEE_ERROR_NOT_SUPPORTED;
	}

	if (!valid_key_size_for_algorithm(algorithm, maxKeySize)) {
		OT_LOG_ERR("TEE_AllocateOperation algorithm [%u] and maxKeySize [%u] combination is not valid", algorithm, maxKeySize);
		return TEE_ERROR_NOT_SUPPORTED;
	}

	if (!supported_algorithms(algorithm, maxKeySize)) {
		OT_LOG_ERR("TEE_AllocateOperation algorithm [%u] AND/OR maxKeySize [%u] not supported", algorithm, maxKeySize);
		return TEE_ERROR_NOT_SUPPORTED;
	}
			       ...
			       TEE_ObjectHandle key)
{
	...
	if (operation == NULL) {
		...
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	} else if (operation->operation_info.handleState == TEE_OPERATION_STATE_ACTIVE) {
		OT_LOG_ERR("TEE_SetOperationKey panicking due operation TEE_OPERATION_STATE_ACTIVE"); 
		TEE_Panic(TEE_ERROR_BAD_PARAMETERS);
	}
