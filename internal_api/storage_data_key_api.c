...
void pri_obj_attr(TEE_ObjectHandle object)
{
	size_t i,j;
	if (object == NULL)
		return;
...
void pri_and_cmp_attr(TEE_ObjectHandle obj1, TEE_ObjectHandle obj2)
{
...
static void release_file(TEE_ObjectHandle object, FILE *obj_file, void *objectID, size_t objectIDLen)
{
...
static void free_attrs(TEE_ObjectHandle object)
{
	...
	for (i = 0; i < object->attrs_count; ++i) {
		if (!is_value_attribute(object->attrs[i].attributeID)) {
			if (object->attrs[i].content.ref.buffer != NULL) {
				...
				if (!RAND_bytes(object->attrs[i].content.ref.buffer, object->maxObjSizeBytes))
	...
	if (object->attrs != NULL) {
		if (!RAND_bytes((unsigned char *)(object->attrs), object->attrs_count * sizeof(TEE_Attribute)))
...
static TEE_Result load_attributes(TEE_ObjectHandle obj)
{
	...
	for (i = 0; i < obj->attrs_count; ++i) {
		if (fread(&obj->attrs[i], sizeof(TEE_Attribute), 1, obj->per_object.object_file) != 1)
			goto err_at_read;
...
static void copy_all_attributes(TEE_ObjectHandle srcObj, TEE_ObjectHandle destObj)
{
	...
	for (i = 0; i < srcObj->attrs_count; i++) {
		if (is_value_attribute(srcObj->attrs[i].attributeID)) {
			...
			memcpy(destObj->attrs[i].content.ref.buffer,
			       srcObj->attrs[i].content.ref.buffer,
			       srcObj->attrs[i].content.ref.length);

			destObj->attrs[i].content.ref.length =
					srcObj->attrs[i].content.ref.length;
		}
	}
}
				      ...
				      TEE_ObjectHandle destObj, uint32_t destIndex)
{
	...
	if (is_value_attribute(srcObj->attrs[srcIndex].attributeID)) {
		...
		memcpy(destObj->attrs[destIndex].content.ref.buffer,
		       srcObj->attrs[srcIndex].content.ref.buffer,
		       srcObj->attrs[srcIndex].content.ref.length);

		destObj->attrs[destIndex].content.ref.length =
				srcObj->attrs[srcIndex].content.ref.length;
	}
...
static TEE_Result gen_rsa_keypair(TEE_ObjectHandle obj, uint32_t key_size, TEE_Attribute* params, uint32_t paramCount)
{
	...
	if (pub_exp_index_at_params != -1) {
		if (BN_bin2bn(params[pub_exp_index_at_params].content.ref.buffer,
			      params[pub_exp_index_at_params].content.ref.length, bn_pub_exp) == 0) {
...
static TEE_Result gen_dsa_keypair(TEE_ObjectHandle object, TEE_Attribute* params, uint32_t paramCount)
{
	int i = 0; /* Attribute index at object */
	int attr_index = 0;
	TEE_Result ret_val = TEE_SUCCESS;
...
static TEE_Result gen_dh_keypair(TEE_ObjectHandle object, TEE_Attribute* params, uint32_t paramCount)
{
	int i = 0; /* Attribute index at object */
	int attr_index = 0;
	TEE_Result ret_val = TEE_SUCCESS;
...
static int get_attr_index_and_check_rights(TEE_ObjectHandle object, uint32_t attributeID)
{
	...
	if (!(attributeID & TEE_ATTR_FLAG_PUBLIC)) {
		syslog(LOG_DEBUG, "Attribute is not public\n");
...
static void delete_object(TEE_ObjectHandle object)
{
	if (object == NULL)
		return;
...
static bool serialize_attributes_to_storage(TEE_ObjectHandle object, FILE *storage)
{
	...
	for (i = 0; i < object->attrs_count; ++i) {
		...
		if (!is_value_attribute(object->attrs[i].attributeID)) {
			...
			ref_attr_size += object->attrs[i].content.ref.length;
		}
	}
	...
	case TEE_TYPE_RSA_KEYPAIR:
		if (get_attr_index_from_attrArr(TEE_ATTR_RSA_PRIME1, attrs, attrCount) != -1 ||
			...
			if (!copy_attr_from_attrArr_to_object(attrs, attrCount, TEE_ATTR_RSA_MODULUS, object, dest_index++) ||
			    ...
			    !copy_attr_from_attrArr_to_object(attrs, attrCount, TEE_ATTR_RSA_PRIME1,  object, dest_index++) ||
...
void TEE_CopyObjectAttributes(TEE_ObjectHandle destObject, TEE_ObjectHandle srcObject)
{
	...
	if (destObject->objectInfo.handleFlags & TEE_HANDLE_FLAG_INITIALIZED ||
		...
		syslog(LOG_ERR, "Destination object initalized and source object is uninitialized\n");
		TEE_Panic(TEE_ERROR_BAD_STATE);
	}
				    ...
				    size_t objectIDLen, uint32_t flags, TEE_ObjectHandle* object)
{
	...
	new_object->objectInfo.maxObjectSize = meta_info_from_storage.maxObjectSize;
	new_object->objectInfo.objectType = meta_info_from_storage.objectType;
	new_object->objectInfo.objectUsage = meta_info_from_storage.objectUsage;
	...
	memcpy(new_object->per_object.obj_id, meta_info_from_storage.obj_id, meta_info_from_storage.obj_id_len);
				      ...
				      size_t initialDataLen, TEE_ObjectHandle* object)
{
	...
	if (attributes != NULL) {
		meta_info_to_storage.handleFlags = attributes->objectInfo.handleFlags;
		meta_info_to_storage.maxObjectSize = attributes->objectInfo.maxObjectSize;
		meta_info_to_storage.objectType = attributes->objectInfo.objectType;
		meta_info_to_storage.objectUsage = attributes->objectInfo.objectUsage;
	...
	if (object != NULL) {
		...
		(*object)->per_object.data_begin = (*object)->per_object.data_position - initialDataLen;
...
err_at_meta_or_init_data_write:
	printf("err_at_meta_or_init_data_write\n");
...
TEE_Result TEE_AllocatePersistentObjectEnumerator(TEE_ObjectEnumHandle* objectEnumerator)
{
	...
	*objectEnumerator = TEE_Malloc(sizeof(struct __TEE_ObjectEnumHandle), 0);
	...
	(*objectEnumerator)->object_info_file = ext_alloc_for_enumerator();
	if ((*objectEnumerator)->object_info_file == NULL)
		goto error;
...
void TEE_FreePersistentObjectEnumerator(TEE_ObjectEnumHandle objectEnumerator)
{
	...
	ext_free_enumerator(objectEnumerator->object_info_file);
...
void TEE_ResetPersistentObjectEnumerator(TEE_ObjectEnumHandle objectEnumerator)
{
	...
	ext_reset_enumerator(objectEnumerator->object_info_file);
}
...
TEE_Result TEE_StartPersistentObjectEnumerator(TEE_ObjectEnumHandle objectEnumerator, uint32_t storageID)
{
	...
	if (!ext_start_enumerator(objectEnumerator->object_info_file))
		return TEE_ERROR_ITEM_NOT_FOUND;
...
{
	struct enumerator_data_chunk recv_per_obj;

	if (objectEnumerator == NULL || objectID == NULL || objectIDLen == NULL)
		return TEE_ERROR_GENERIC;

	if (!ext_get_next_obj_from_enumeration(objectEnumerator->object_info_file))
		return TEE_ERROR_ITEM_NOT_FOUND;

	rewind(objectEnumerator->object_info_file);

	/* We received info from manager */
	memset(&recv_per_obj, 0, sizeof(struct enumerator_data_chunk));
	if (fread(&recv_per_obj, sizeof(struct enumerator_data_chunk), 1,
		  objectEnumerator->object_info_file) != 1) {
		syslog(LOG_ERR, "Cannot read file (getNextPerObj); errno: %i\n", errno);
		return TEE_ERROR_GENERIC;
	}
	...
	*objectIDLen = recv_per_obj.obj_id_len;
	...
	if (objectInfo != NULL) {
		objectInfo->objectType = recv_per_obj.objectType;
		objectInfo->objectSize = recv_per_obj.objectSize;
		objectInfo->maxObjectSize = recv_per_obj.maxObjectSize;
		objectInfo->objectUsage = recv_per_obj.objectUsage;
		if (recv_per_obj.dataSize > UINT32_MAX)
			objectInfo->dataSize = UINT32_MAX;
		else
			objectInfo->dataSize = recv_per_obj.dataSize;
		objectInfo->dataPosition = 0;
		objectInfo->handleFlags = recv_per_obj.handleFlags;
	}
...
error:
	 ...
	 * TODO: add fseek err check */
	fseek(object->per_object.object_file, object->per_object.data_position, SEEK_SET);
	if (err_no == ENOSPC)
		return TEE_ERROR_STORAGE_NO_SPACE;
...
TEE_Result TEE_TruncateObjectData(TEE_ObjectHandle object, uint32_t size)
{
	...
	if (pos > (size + begin)) {
		...
		fseek(object->per_object.object_file, size + begin, SEEK_SET);
