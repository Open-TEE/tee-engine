...
#define SECURE_STORAGE_PATH "/tmp/TEE_secure_storage/"
#endif

static const uint32_t EMU_ALL = 0xF000001F; //rename
...
static int is_directory_empty(char *dir_path)
{
	struct dirent *entry;
	int file_count = 0;
	DIR *dir = opendir(dir_path); //NULL case
	if (dir == NULL)
		return 0;
...
{
	//add nulkl character and NULL chcek
...
static FILE *query_for_access(void *objectID, size_t objectIDLen, size_t seek_access)
{
	...
	char UUID[sizeof(TEE_UUID)]; //+1 size
	DIR *dir;

	get_uuid(UUID);

	//snprintf and check objec_len
	...
	//else if and log open error
	if (seek_access & TEE_DATA_FLAG_ACCESS_WRITE_META ||
	    seek_access & TEE_DATA_FLAG_ACCESS_WRITE) {
		remove(name_with_dir_path); /* testing and remove */
...
//break separate file (mimic funcs)
//add same cor to this
...
//Should this be bool
static int copy_obj_attr_to_obj(TEE_ObjectHandle srcObj, uint32_t attrID, TEE_ObjectHandle destObj,
				uint32_t destIndex)
{
	int srcIndex = -1;
	uint32_t i;

	//add buffer null check, obj
	//move to own func
	if (attrID == EMU_ALL) {
...
//rename to copy
//Should bool
static int extract_attr_to_object(uint32_t ID, TEE_Attribute* params, uint32_t paramCount,
				  TEE_ObjectHandle object, uint32_t index)
{
	int attr_index;
	...
	if (attr_index == -1)
		return -1; /* Array does not contain extracted attribute */
	...
	if (!is_value_attribute(params[attr_index].attributeID)) {
		//copy just maxbyte size
		if (object->maxObjSizeBytes >= params[attr_index].content.ref.length) {
			memcpy(object->attrs[index].content.ref.buffer,
...
{
	//seed
	//create macro for keysize to bytes size
...
//check if public exponent is passed
static int gen_rsa_keypair(TEE_ObjectHandle obj, uint32_t key_size)
{
	int i = 0;
	RSA *rsa_key = RSA_generate_key(key_size, RSA_F4, NULL, NULL);
...
static int gen_dsa_keypair(TEE_ObjectHandle object, TEE_Attribute* params, uint32_t paramCount)
{
	...
	BN_bin2bn(object->attrs[attr_index].content.ref.buffer,
...
void TEE_CloseObject(TEE_ObjectHandle object)
{
	if (object->objectInfo.handleFlags & TEE_HANDLE_FLAG_PERSISTENT) {
		...
		fclose(object->per_obj_file_handler); /* move to manager file */
		free_attrs(object);
		free(object->attrs);
				    ...
				    size_t objectIDLen, uint32_t flags, TEE_ObjectHandle* object)
{
	...
	if (fread(&tmp_persistant_object, sizeof(struct persistant_object), 1,
		  tmp_handle->per_obj_file_handler) != 1)
		goto err_at_meta_read;

	memcpy(&tmp_handle->objectInfo, &tmp_persistant_object.info,
	       sizeof(TEE_ObjectInfo));
	tmp_handle->attrs_count = tmp_persistant_object.attrs_count;
	
	if (tmp_handle->attrs_count > 0) {
		//check maxobjecsize val
		...
		if (ret_val == TEE_ERROR_OUT_OF_MEMORY || ret_val == TEE_ERROR_GENERIC) {
			free(tmp_handle); //file leak
			return ret_val;
		}
	}

	data_begin = ftell(tmp_handle->per_obj_file_handler);
	fseek(tmp_handle->per_obj_file_handler, 0, SEEK_END); /* check if portable */
				      ...
				      size_t initialDataLen, TEE_ObjectHandle* object)
{
	...
	if (attributes != NULL && attributes->attrs_count > 0) {
		for (i = 0; i < attributes->attrs_count; ++i) {
			if (fwrite(&attributes->attrs[i], sizeof(TEE_Attribute), 1,
				   tmp_per_obj_file) != 1)
				goto err_at_meta_data_write;

			if (!is_value_attribute(attributes->attrs[i].attributeID))
				if (fwrite(attributes->attrs[i].content.ref.buffer,
					   attributes->attrs[i].content.ref.length, 1,
					   tmp_per_obj_file) != 1)
					goto err_at_meta_data_write;
		}
	}
	...
	if (object != NULL) {
		*object = TEE_Malloc(sizeof(struct __TEE_ObjectHandle), 0);
		...
		memcpy(*object, attributes, sizeof(struct __TEE_ObjectHandle));
		(*object)->per_obj_file_handler = tmp_per_obj_file;

		if ((*object)->attrs_count > 0) {
			ret_val = load_attributes((*object)); //not work, no malloc done
			if (ret_val == TEE_ERROR_OUT_OF_MEMORY || ret_val == TEE_ERROR_GENERIC) {
				free(*object);
				return ret_val;
			}
		...
		(*object)->data_begin = ftell((*object)->per_obj_file_handler); //move end, end to data
...
TEE_Result TEE_RenamePersistentObject(TEE_ObjectHandle object, void* newObjectID, size_t newObjectIDLen)
{
	...
	fseek(object->per_obj_file_handler, object->data_begin, SEEK_SET); //set cur pos
...
TEE_Result TEE_WriteObjectData(TEE_ObjectHandle object, void* buffer, size_t size)
{
	...
	//log error, Truncate the written bytes and no space
...
TEE_Result TEE_TruncateObjectData(TEE_ObjectHandle object, uint32_t size)
{
	...
	ftruncate(fileno(object->per_obj_file_handler), object->data_begin + size);

	if (object->objectInfo.dataPosition > (size + object->data_begin))
...
TEE_Result TEE_SeekObjectData(TEE_ObjectHandle object, int32_t offset, TEE_Whence whence)
{
	...
	data_begin = object->data_begin;
	data_end = object->objectInfo.dataSize + data_begin;
	data_pos = object->objectInfo.dataPosition;

	if (whence == TEE_DATA_SEEK_CUR) {
		if ((data_pos + offset) > (data_end))
			data_pos = data_end;
		else if ((data_pos + offset) < data_begin)
			data_pos = data_begin;
		else
			data_pos += offset;
		...
		if (offset > 0)
			data_pos = data_end;
		else if ((data_end + offset) < data_begin) //negative offset
			data_pos = data_begin;
		else
			data_pos = data_end + offset;
	} else if (whence == TEE_DATA_SEEK_SET) {
		if (offset < 0)
			data_pos = data_begin;
		else if ((data_begin + offset) > data_end)
			data_pos = data_end;
		else
			data_pos += offset;
	} else {
	...
	object->objectInfo.dataPosition = data_pos;

	fseek(object->per_obj_file_handler, data_pos, SEEK_SET);
