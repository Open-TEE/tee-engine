				    ...
				    struct gp_attributes *attributes)
{
	...
	offset += memcpy_ret_n(&attributes->attrs_count, mem_in, sizeof(attributes->attrs_count));
	mem_in += offset;
	...
	for (n = 0; n < attributes->attrs_count; ++n) {
		offset += memcpy_ret_n(attributes->attrs[n].attributeID, mem_in, sizeof(attr.attributeID));
		mem_in += offset;

		if (is_value_attribute(attr.attributeID)) {
			offset += memcpy_ret_n(&attributes->attrs[n].content.value.a, mem_in, sizeof(attr.content.value.a));
			mem_in += offset;

			offset += memcpy_ret_n(&attributes->attrs[n].content.value.b, mem_in, sizeof(attr.content.value.b));
			mem_in += offset;
		} else {
			offset += memcpy_ret_n(&attributes->attrs[n].content.ref.length, mem_in, sizeof(attr.content.ref.length));
			mem_in += offset;
			...
			offset += memcpy_ret_n(attributes->attrs[n].content.ref.buffer,
			    ...
			    char *mem_in)
{
	...
	offset += memcpy_ret_n(mem_in, &attr->attributeID, sizeof(attributes->attrs_count));
	mem_in += offset;
