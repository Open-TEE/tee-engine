 ...
 * \param public_sockfd The main socket to which clients connect
 * \return 0 on success -1 otherwise
 */
static int init_sock(int *public_sockfd)
{
	...
	*public_sockfd = socket(AF_UNIX, SOCK_STREAM, 0);
	if (*public_sockfd == -1) {
	...
	if (bind(*public_sockfd, (struct sockaddr *) &sock_addr, sizeof(struct sockaddr_un)) == -1) {
		syslog(LOG_ERR, "Error %s", strerror(errno));
		return -1;
	}

	if (listen(*public_sockfd, SOMAXCONN) == -1) {
...
int daemon_main_loop(sig_status_cb check_signal_status)
{
	...
	for (;;) {
		...
		status = wrap_epoll_wait(cur_events, MAX_CURR_EVENTS);
		if (status == -1) {
			if (errno == EINTR) {
